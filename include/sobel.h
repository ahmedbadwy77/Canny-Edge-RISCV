...
void sobel_gradients_rvv(const uint8_t* input,
    ...
    // boundary checks inside the vectorized inner loop.
    for (int y = 1; y < height - 1; y++) {
// --- Vector-length-agnostic Inner Loop Starts Here ---
...
void sobel_gradients_rvv(const uint8_t* input, int16_t* grad_x, int16_t* grad_y, int width, int height) {
    for (int y = 1; y < height - 1; y++) {
        for (int x = 1; x < width - 1; ) {
            size_t remaining = (width - 1) - x;
            ...
            vuint16m2_t top_u16 = __riscv_vzext_vf2_u16m2(top, vl+2);
            vuint16m2_t mid_u16 = __riscv_vzext_vf2_u16m2(mid, vl+2);
            vuint16m2_t bot_u16 = __riscv_vzext_vf2_u16m2(bot, vl+2);

            vint16m2_t top16 = __riscv_vreinterpret_v_u16m2_i16m2(top_u16);
            vint16m2_t mid16 = __riscv_vreinterpret_v_u16m2_i16m2(mid_u16);
            vint16m2_t bot16 = __riscv_vreinterpret_v_u16m2_i16m2(bot_u16);

            vint16m2_t top_left   = __riscv_vslide1up_vx_i16m2(top16, 0, vl+2);
            vint16m2_t mid_left   = __riscv_vslide1up_vx_i16m2(mid16, 0, vl+2);
            vint16m2_t bot_left   = __riscv_vslide1up_vx_i16m2(bot16, 0, vl+2);

            vint16m2_t top_right  = __riscv_vslide1down_vx_i16m2(top16, 0, vl+2);
            vint16m2_t mid_right  = __riscv_vslide1down_vx_i16m2(mid16, 0, vl+2);
            vint16m2_t bot_right  = __riscv_vslide1down_vx_i16m2(bot16, 0, vl+2);
